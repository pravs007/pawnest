...
export const validateName = (name, fieldLabel = 'Name') => {
  ...
  // Letters and spaces only (including common letters/punctuation like accents/apostrophes is fine, but standard is letters & spaces)
  if (!/^[a-zA-Z\s]+$/.test(trimmed)) {
    return `${fieldLabel} must contain only letters and spaces`;
  }
  // Prevent repeated special characters (e.g., consecutive spaces)
  if (/\s{2,}/.test(trimmed)) {
